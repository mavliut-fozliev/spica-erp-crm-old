...
export default function useAutoUpdateAssemblyPlan() {
  ...
  const autoUpdateUpdateAssemblyPlan: TAutoUpdateFunction = (isUpdate, idsToDelete, hasWarning) => {
    ...
    const currentTeamTasks = tasksWithoutCurrent.filter((row) => getIdValueFromObject(row.field_team) === getIdValueFromObject(formData.field_team));
  ...
  return autoUpdateUpdateAssemblyPlan;
}
