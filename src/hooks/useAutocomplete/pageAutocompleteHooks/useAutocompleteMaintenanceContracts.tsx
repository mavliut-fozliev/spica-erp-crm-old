...
export default function useAutocompleteMaintenanceContracts(processAndSaveFormData: TprocessAndSaveFormData, shouldPasteFromStore: TshouldPasteFromStore) {
  const { getPageDocument } = useGetStoredPageData();

  const autocompleteMaintenanceContracts: TAutocompleteFunction = (newFormData) => {
    const getCustomer = () => {
  ...
  return autocompleteMaintenanceContracts;
}
