...
export default function useAutocompleteAssemblyPlan(processAndSaveFormData: TprocessAndSaveFormData, shouldPasteFromStore: TshouldPasteFromStore) {
  const { getPageDocuments, getPageDocument } = useGetStoredPageData();
  const instantStore = useInstantStore();

  const autocompleteAssemblyPlan: TAutocompleteFunction = (newFormData) => {
    const getCustomer = () => {
    ...
    const currentTeamItems = pageData.filter((row) => getIdValueFromObject(row.field_team) === getIdValueFromObject(newFormData.field_team));
    ...
    const start_date = newFormData.start_date || findNextWorkDay(lastEnd);
    ...
    const end_date = findEndDate(start_date, amount_of_days);
  ...
  return autocompleteAssemblyPlan;
}
