...
const { convertTimeToShow } = require('../../../services/functions/helpers/dateHelpers');
const { ViewMode } = require('./index.modern');
const { getFeastHolidays } = require('../../../services/functions/getFeastHolidays/getFeastHolidays');
...
var ganttDateRange = function ganttDateRange(tasks, viewMode, preStepsCount) {
  ...
  for (var _iterator = _createForOfIteratorHelperLoose(tasks), _step; !(_step = _iterator()).done;) {
    ...
    if (task.end > newEndDate) {
      ...
      newEndDate = new Date(endDate.setMonth(task.end.getMonth() + 20));
    }
  }
...
var styles$4 = {"gridRow":"_2dZTy","gridRowLine":"_3rUKi","gridTick":"_RuwuK", "weekend": "_weekend"};
...
var GridBody = function GridBody(_ref) {
  ...
  var feastHolidays = getFeastHolidays(currentYear)
  ...
  for (var i = 0; i < dates.length; i++) {
    ...
    if (currentYear !== dayjs(date).format("YYYY")){
      currentYear = dayjs(date).format("YYYY")
      feastHolidays = getFeastHolidays(currentYear)
    }

    const dayIsHoliday = date.getDay() === 0 || holidays.includes(dayjs(date).format("MM-DD")) || feastHolidays.includes(dayjs(date).format("MM-DD")) 
    ...
    weekends.push(React__default.createElement("rect", {
      ...
      x: tickX,
      y: 0,
      width: viewModeGlobal === 'Day' && dayIsHoliday ? 20 : 0,
      height: y,
      className: styles$4.weekend
    }));

    if (i + 1 !== dates.length && date.getTime() < now.getTime() && dates[i + 1].getTime() >= now.getTime() || i !== 0 && i + 1 === dates.length && date.getTime() < now.getTime() && addToDate(date, date.getTime() - dates[i - 1].getTime(), "millisecond").getTime() >= now.
    getTime()) {
      today = React__default.createElement("rect", {
        x: tickX,
        y: 0,
        width: viewModeGlobal === 'Day' ? 20 : viewModeGlobal === 'Week' ? 26 :  columnWidth,
        height: y,
        fill: todayColor 
      });
    }
    ...
    tickX += viewModeGlobal === 'Day' ? 20 : viewModeGlobal === 'Week' ? 26 :  columnWidth;
  }
  ...
  return React__default.createElement("g", {
    ...
    className: "weekends"
  }, weekends), React__default.createElement("g", {
    className: "today"
  }, today));
};
...
var Calendar = function Calendar(_ref) {
  ...
  var getCalendarValuesForWeek = function getCalendarValuesForWeek() {
    ...
    const customWidth = 26;

      var bottomValue = date.getDate();
      bottomValues.push(React__default.createElement("text", {
        key: date.getTime(),
        y: headerHeight * 0.8,
        x: customWidth * (i + rtl),
      ...
      if (topValue) {
        if (i !== dates.length - 1) {
          topValues.push(React__default.createElement(TopPartOfCalendar, {
            key: topValue,
            value: topValue,
            x1Line: customWidth * i + weeksCount * customWidth,
            y1Line: 0,
            y2Line: topDefaultHeight,
            xText: customWidth * i + customWidth * weeksCount * 0.5,
            yText: topDefaultHeight * 0.9
          }));
        }
  ...
  var getCalendarValuesForDay = function getCalendarValuesForDay() {
    ...
    for (var i = 0; i < dates.length; i++) {
      ...
      bottomValues.push(React__default.createElement("text", {
        key: date.getTime(),
        y: headerHeight * 0.9,
        x: customWidth * i + customWidth * 0.5,
        className: styles$5.calendarBottomText
      }, bottomValue));

      var daysOfWeek = ["Pz", "Pt", "Sl", "Çr", "Pş", "Cm", "Ct"] 
      ...
      if (i + 1 !== dates.length && date.getMonth() !== dates[i + 1].getMonth()) {
        ...
        topValues.push(React__default.createElement(TopPartOfCalendar, {
          ...
          value: topValue,
          x1Line: customWidth * (i + 1),
          y1Line: 0,
          y2Line: topDefaultHeight,
          xText: customWidth * (i + 1) - getDaysInMonth(date.getMonth(), date.getFullYear()) * customWidth * 0.5,
...
var convertToBar = function convertToBar(task, index, dates, columnWidth, rowHeight, taskHeight, barCornerRadius, handleWidth, rtl, barProgressColor, barProgressSelectedColor, barBackgroundColor, barBackgroundSelectedColor) {
  ...
  const customWidthForDay = 20
  const customWidthForWeek = 26
  
  if (rtl) {
    x2 = taskXCoordinateRTL(task.start, dates, viewModeGlobal === 'Day' ? customWidthForDay : viewModeGlobal === 'Week' ? customWidthForWeek : columnWidth);
    x1 = taskXCoordinateRTL(task.end, dates, viewModeGlobal === 'Day' ? customWidthForDay : viewModeGlobal === 'Week' ? customWidthForWeek : columnWidth);
  } else {
    x1 = taskXCoordinate(task.start, dates, viewModeGlobal === 'Day' ? customWidthForDay : viewModeGlobal === 'Week' ? customWidthForWeek : columnWidth);
    x2 = taskXCoordinate(task.end, dates, viewModeGlobal === 'Day' ? customWidthForDay : viewModeGlobal === 'Week' ? customWidthForWeek : columnWidth);
  }
...
var Bar = function Bar(_ref) {
  ...
  }, isDateChangeable && React__default.createElement("g", {className: "task"}, React__default.createElement(BarDateHandle, {
    x: task.x1 + 1,
    y: task.y + 1,
...
var Gantt = function Gantt(_ref) {
  ...
  var handleMouseDownTask = function handleMouseDownTask(taskId) {
    var newSelectedTask = barTasks.find(function (t) {
      return t.id === taskId;
    });
        onMouseDown(newSelectedTask, true);
  };
